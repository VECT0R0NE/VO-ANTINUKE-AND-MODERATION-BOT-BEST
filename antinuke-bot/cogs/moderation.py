...
class Moderation(commands.Cog):
    ...
    async def lockdown(self, interaction: discord.Interaction,
        ...
        # Also deny the configured member/verified role if set
        member_role_id = await self.bot.db.get_lockdown_member_role(interaction.guild.id)
        if member_role_id:
            member_role = interaction.guild.get_role(member_role_id)
            if member_role:
                member_ow = target.overwrites_for(member_role)
                member_ow.send_messages = False
                try:
                    await target.set_permissions(member_role, overwrite=member_ow,
                                                 reason=f"Lockdown (member role) by {interaction.user}: {reason}")
                except Exception:
                    pass
        ...
        embed.add_field(name="📋 Reason", value=reason, inline=False)
        if member_role_id and interaction.guild.get_role(member_role_id):
            embed.add_field(name="🎭 Member Role Locked", value=interaction.guild.get_role(member_role_id).mention, inline=True)
        embed.set_footer(text=f"{BOT_NAME} • Moderation", icon_url=interaction.guild.me.display_avatar.url)
        await interaction.followup.send(embed=embed)
    ...
    async def unlockdown(self, interaction: discord.Interaction,
        ...
        # Also restore the configured member/verified role
        member_role_id = await self.bot.db.get_lockdown_member_role(interaction.guild.id)
        if member_role_id:
            member_role = interaction.guild.get_role(member_role_id)
            if member_role:
                member_ow = target.overwrites_for(member_role)
                member_ow.send_messages = None
                try:
                    await target.set_permissions(member_role, overwrite=member_ow,
                                                 reason=f"Unlock (member role) by {interaction.user}: {reason}")
                except Exception:
                    pass
    ...
    async def masslockdown(self, interaction: discord.Interaction, reason: str = "No reason provided"):
        ...
        for ch in text_channels:
            try:
                ...
                # Lock member role if configured
                if member_role:
                    mow = ch.overwrites_for(member_role)
                    mow.send_messages = False
                    await ch.set_permissions(member_role, overwrite=mow,
                                             reason=f"Mass lockdown (member role) by {interaction.user}: {reason}")
                locked += 1
            except Exception:
                failed += 1
    ...
    async def massunlockdown(self, interaction: discord.Interaction, reason: str = "No reason provided"):
        ...
        for ch in text_channels:
            try:
                ...
                # Restore member role too
                if member_role:
                    mow = ch.overwrites_for(member_role)
                    mow.send_messages = None
                    await ch.set_permissions(member_role, overwrite=mow,
                                             reason=f"Mass unlockdown (member role) by {interaction.user}: {reason}")
                unlocked += 1
            except Exception:
                failed += 1
            await asyncio.sleep(0.3)
